...
def current_date () :
    ...
    cyear = datetime.datetime.utcnow().year
    cmonth = datetime.datetime.utcnow().month
    cday = datetime.datetime.utcnow().day
    cdate = cyear+cmonth/100+cday/10000
    return cdate 
...
def hms (tm):
    ...
    if float(tm) < 0:          #if number is negative
        flag1 = 1       #flag set to 1 means number
...
def current_time() :
    chour = datetime.datetime.utcnow().hour
    cminute = datetime.datetime.utcnow().minute
    csecond = datetime.datetime.utcnow().second
    cmicrosecond = datetime.datetime.utcnow().microsecond/1000000
    ctime=chour+cminute/100+csecond/10000+cmicrosecond/10000
    return ctime

def is_Number(name,hour,loH,hiH,minute,loM,hiM,second,loS,hiS,t):
    running = True
    while running:
        temp=input(name)
        if temp=='e' : 
            running=False
        if len(temp)==0 and t==1 :
...
tempt=current_time()
tempd=current_date()
docname=(str(tempd) + '.' + "{:.6f}".format(tempt))

title=input('enter name of text file ')
if title=='':# if no title was entered then this 'Celestial_position.docname.txt' will be the saved text
    from random import randint
    title=('Celestial_position'+ docname)# if no title was entered then this 'Celestial_position.docname.txt' will be the saved text
print ('text file will be',title+'.txt')
text_file = open (title+'.txt','x')
text_file.close()

date,yr,mn,dy = date_leap_year('input date yyyy.mmdd ','year',1583,5000,'month','day')
print ('date ',date,'=',yr+'/'+mn+'/'+dy)
time,h,m,s,ms,dtm = is_Number('input time h.mmssss UT ','hour',0,23,'minute',0,59,'seconds',0,59,1)
print ('time ',time+' = '+h+':'+m+':'+s+'.'+ms,'  decimal time %.6f'%dtm)



latitude,latd,latm,lats,latms,dlatitude = is_Number('latitude ','degrees',-90,90,'arc minutes',0,59,'arc seconds',0,59,0)
print ('latitude ',latitude,'=',latd+u'\N{DEGREE SIGN}',latm+'\'',lats+'"','  decimal %.6f'%dlatitude)
longitude,lond,lonm,lons,ms,dlongitude = is_Number('longitude ','degrees',-180,180,'arc minutes',0,59,'arc seconds',0,59,0) 
print ('longitude ',longitude,'=',lond+u'\N{DEGREE SIGN}',lonm+'\'',lons+'"','  decimal %.6f'%dlongitude)
declination,decd,decm,decs,ms,ddec=is_Number('declination d.mmss ','degree',-90,90,'arc minutes',0,59,'arc seconds',0,59,0)
print ('declination ',declination,'=',decd+u'\N{DEGREE SIGN}',decm+'\'',decs+'"','  decimal %.6f'%ddec)
Right_ascention,RAh,RAm,RAs,ms,dRA = is_Number('right ascention ','hour',0,23,'arc minutes',0,59,'arc seconds',0,59,0)
print ('Right ascention ',Right_ascention,'=',RAh+'h',RAm+'\'',RAs+'"','  decimal %.6f'%dRA )
print ()
...
#    ddt,y,m,d,ms = hms(date)
    ...
    text_file = open (title+'.txt','a')
    text_file.write('############################################')
    text_file.write('\n')
    text_file.write('time UT         ' + str(h) + ':' + str(m) + ':' + str(s))
    text_file.write('\n')
    text_file.write('date            ' + str(yr) + '/' + str(mn) + '/' + str(dy))
    text_file.write('\n')
    text_file.write('latitude        ' + str(latd) +u'\N{DEGREE SIGN}' + str(latm) + '\'' + str(lats) +'"' )
    text_file.write('\n')    
    text_file.write('longitude       ' + str(lond) + u'\N{DEGREE SIGN}' + str(lonm) +'\''+ str(lons) +'"' )
    text_file.write('\n')    
    text_file.write('declination     ' + str(decd) + u'\N{DEGREE SIGN}' + str(decm) + '\'' + str(decs) + '"'  )
    text_file.write('\n')    
    text_file.write('Right ascention ' + str(RAh) +'h' + str(RAm) +'\'' + str(RAs) + '"'  )
    text_file.write('\n')
...
#    print('decimal       %.8f' % LSdtm)
    print ()
    text_file.write('Local Sideral   ' + str(LSh) + ':' + str(LSm) + ':' + str(LSs))
    text_file.write('\n')
...
#    print ('azmuth          ',azd + u'\N{DEGREE SIGN}' + azm + '\'' + azs + '.' + azms + '"')
    ...
    text_file.write('altitude        ' + str("{:.2f}".format(dal))  + u'\N{DEGREE SIGN}' )
    text_file.write('\n')
    text_file.write('azmuth          ' + str("{:.2f}".format(daz)) + u'\N{DEGREE SIGN}' )
    text_file.write('\n')
    text_file.write('############################################') 
    text_file.write('\n')
    text_file.write('\n')  
    ...
    time,h,m,s,ms,dtm = is_Number('exit <e> current time <enter> input h.mmssss UT ','hour',0,23,'minute',0,59,'seconds',0,59,1)
